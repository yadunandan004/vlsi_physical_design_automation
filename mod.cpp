...
map<int,map<int,vector<string> > >gainBucket;
...
//partition created such that it is in order with ratio factor 
void createPartition()
{
...
bool areaSort(string cell1,string cell2)
{
	if(cellData[cell1].area<cellData[cell2].area)
	{
		return true;
	}
	else
...
void createGainBucket()
{
	...
	for(int i=0;i<cellIds.size();i++)
	{
		...
		if(cellData[cellId].getLockStatus()==false)
		{
			...
			map<int,map<int,vector<string> > >::iterator pos;
			...
			if(pos==gainBucket.end())
			{
				...
				vector <string> empty;
				gainBucket.insert(pair<int, map<int,vector<string> > >(gain,temp));
				gainBucket[gain].insert(pair<int,vector<string> >((int)cellData[cellId].partition,cellist));
				gainBucket[gain].insert(pair<int,vector<string> >((int)cellData[cellId].partition^1, empty));
				// gainBucket.insert(pair<int,vector<string> >(gain,cellist));
			}
			//if gain found just append cell to it
			else{
				pos->second[cellData[cellId].partition].push_back(cellId);
				sort(pos->second[cellData[cellId].partition].begin(),pos->second[cellData[cellId].partition].end(),areaSort);
				// for(int i=0;i<pos->second[cellData[cellId].partition].size();i++)
				// {
				// 	if(cellData[pos->second[cellData[cellId].partition].at(i)].area>cellData[cellId].area)
				// 	{
				// 		pos->second[cellData[cellId].partition].insert(pos->second[cellData[cellId].partition].begin()+i,cellId);
				// 	}
				// }
			}
		}
		...
		for(int j=0;j<net.size();j++)
		{
			...
			if(cellId!=node)
			{
				if(cellData[node].getLockStatus()==false)
				{
					...
					int npar=cellData[node].partition;
					//find the position of the node in the gain bucket and remove it
					// std::vector<string>::iterator pos=gainBucket[ngain].find(node);
					for(int k=0;k<gainBucket[ngain][npar].size();k++)
					{
						if(gainBucket[ngain][npar].at(k)==node)
						{
							gainBucket[ngain][npar].erase(gainBucket[ngain][npar].begin()+k);
							//compute new gain and add it to the new position in the gain bucket
							int newGain=computeGain(node);
							cellData[node].gain=newGain;
							gainBucket[newGain][npar].push_back(node);
							// for(int i=0;i<gainBucket[newGain][npar].size();i++)
							// {
							// 	if(cellData[gainBucket[newGain][npar].at(i)].area>cellData[node].area)
							// 	{
							// 		gainBucket[newGain][npar].insert(gainBucket[newGain][npar].begin()+i,node);
							// 	}
							// }
							sort(gainBucket[newGain][npar].begin(),gainBucket[newGain][npar].end(),areaSort);
							break;
						}
					}
				}
...
void moveCells()
{
		...
		map<int,map<int,vector<string> > >::reverse_iterator it1 =gainBucket.rbegin();
		while(it1 !=gainBucket.rend())
		{
			...
			for(int j=0;j<2;j++)
			{
				...
				for(int i=0;i<(it1->second)[j].size();i++)
				{
					string node=(it1->second)[j].at(i);
					if(cellData[node].getLockStatus()==false)
					{
						...
						else
						{
							...
							gainBucket[it1->first][j].erase(find(gainBucket[it1->first][j].begin(),gainBucket[it1->first][j].end(),node));
							flag=true;
							break;
						}
					}
